...
contract.only("ExampleContract", function ([owner, revealee, random, node]) {
    ...
    describe("requestVerification()", function () {
        it("should revert if caller is not the owner", async function() {
            await expectRevert(this.example.requestVerification(revealee, {from: random}), "Ownable: caller is not the owner");
        });

        it("should revert if insufficient allowance", async function() {
            await expectRevert(this.example.requestVerification(revealee, {from: owner}), "SafeERC20: low-level call failed");
        });
        ...
        describe("after approving", function () {
           ...
           it("should request if all conditions are met", async function () {
               ...
               expect(await this.link.balanceOf(owner)).to.be.bignumber.equal(balanceBefore.sub(new BN(oraclePayment)));
