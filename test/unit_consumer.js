...
contract("UnitConsumer", function([owner, stranger, addr, ...accounts]) {
    ...
    describe("#setLink", async function () {
        it("should set properly with owner sender", async function () {
            await this.consumer.setLink(addr, {from: owner});
            expect(await this.consumer.linkAddress()).to.be.equal(addr);
        });

        it("should revert if sender is not an owner", async function () {
            await expectRevert(
                this.consumer.setLink(addr, {from: stranger}),
                'Ownable: caller is not the owner'
            );
        });
    });
